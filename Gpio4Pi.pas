...
type
  ...
  protected
    ...
    PUartMem: ^LongWord;     // Mapped UART mem.  Only in /dev/mem
  ...
  public
    ...
    // ClkNo: CLK_GPIO0, CLK_GPIO1, CLK_GPIO2, CLK_PCM, CLK_PWM, CLK_UART
    // Data:  Data returned to the caller:
...
begin
  ...
  // Set PWM Clock
  Result:= SetBaseClock(CLK_UART_CTL, CLK_UART_DIV, Freq);
end;
...
begin
  ...
  begin
    ...
    pClk^:= BCM_PASSWORD;                            // Set source to GND
    while (pClk^ and $00000080) <> 0 do              // Wait for clock to be not BUSY
      DelayMicroSeconds(10);
...
begin
  ...
  case ClkNo of
    ...
    CLK_UART:  begin pCtl:= SetPtr(PClkMem, CLK_UART_CTL); pDiv:= SetPtr(PClkMem, CLK_UART_DIV); end;
    else Exit(False);
  end;
